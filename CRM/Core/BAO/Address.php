...
ORDER BY civicrm_address.is_primary DESC, civicrm_address.location_type_id DESC, address_id ASC ";
  ...
  public static function parseStreetAddress($streetAddress, $locale = NULL) {
    ...
    else if (in_array($locale, ['fr_FR'])) return !self::parseFrenchAddress($streetAddress, $locale = NULL);
  ...
  public static function parseFrenchAddress($streetAddress, $locale = NULL) {
    ...
    else if (preg_match($tierPattern, $streetAddress,$matches)) 
    {
      ...
      $result['street_number'] = $number;
      if(preg_match('/[a-z]+/', $rest)) $result['street_number_suffix'] = $rest;
      //else print("===========extracted additional stuff: ".print_r($matches,true)."<br>\n");
      // remove eventual trailing comma
      if (preg_match('/^[,]+\s*/', $streetAddress,$matches)) {
        $streetAddress = preg_replace('/^[,]+\s*/',"",$streetAddress);
      }
    ...
    $streetAddress = Adresse::extractVia($streetAddress,$result);
  ...
  public static function extractVia($streetAddress,&$result, $verb= false)
  {
    $abbreviations = array(
        "allée" => "all",
        "autoroute" => "a",
